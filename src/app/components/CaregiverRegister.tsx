...
export function CaregiverRegister() {
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    toast.success(`¡Cuenta creada, ${formData.name.split(" ")[0]}!`, { description: "Bienvenido a AMAY." });
    navigate("/caregiver/dashboard");
  };

  const handleSocialLogin = (provider: string) => {
    toast.success(`Cuenta creada con ${provider}`, { description: "Entrando a tu panel... (Demo)" });
    setTimeout(() => navigate("/caregiver/dashboard"), 800);
  };

  return (
    <div className="flex-1 flex flex-col bg-slate-50 h-full overflow-y-auto pb-10" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      {/* Header */}
      <div className="bg-[#2B59FF] text-white px-6 pt-10 pb-12 rounded-b-[2.5rem] shadow-lg relative z-20">
        <div className="flex justify-between items-center mb-4">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Registro</h1>
          <div className="w-12 h-12" />
        </div>
        <p className="text-blue-100 text-base leading-relaxed opacity-90">
          Crea tu cuenta para empezar a gestionar la salud de tus seres queridos.
        </p>
      </div>
      ...
      <div className="px-6 -mt-6 relative z-30 space-y-6">
        ...
        <div className="relative py-2">
          ...
          <div className="relative flex justify-center">
            <span className="bg-slate-50 px-4 text-sm font-bold text-slate-400">O regístrate con</span>
          </div>
        </div>
        ...
        <p className="text-center text-sm font-semibold text-slate-400">
          ...
          <button onClick={() => navigate("/caregiver/login")} className="text-[#2B59FF] font-bold">
            Inicia sesión
          </button>
        </p>
      </div>
